...
if __name__ == "__main__":
    ...
    current_x_position = pose1.position.x
    current_y_position = pose1.position.y
    current_z_orientation = pose1.orientation.z
    ...
    while not rospy.is_shutdown():
        ...
        for i in range(0, len(poses_list) - 1):
            ...
            for i in range(0, poses_list[i][1]):
                ...
                t.transform.rotation.x = tf.transformations.quaternion_from_euler(
                    0, 0, current_z_orientation
                )[0]
                t.transform.rotation.y = tf.transformations.quaternion_from_euler(
                    0, 0, current_z_orientation
                )[1]
                t.transform.rotation.z = tf.transformations.quaternion_from_euler(
                    0, 0, current_z_orientation
                )[2]
                t.transform.rotation.w = tf.transformations.quaternion_from_euler(
                    0, 0, current_z_orientation
                )[3]
                ...
                current_z_orientation += diff_z_orientation

                # print("publishing transform from python")
                rate.sleep()
